...
BEGIN
  INSERT INTO public.user_credits (user_id, credits_remaining, credits_used) VALUES (NEW.id, 50, 0);
  RETURN NEW;
...
DECLARE v_last_reset timestamp with time zone; v_was_reset boolean := false;
BEGIN
  SELECT uc.last_reset_at INTO v_last_reset FROM public.user_credits uc WHERE uc.user_id = uid;
  IF v_last_reset IS NOT NULL AND v_last_reset < now() - interval '1 month' THEN
    UPDATE public.user_credits uc SET credits_remaining = 50, credits_used = 0, last_reset_at = now(), updated_at = now() WHERE uc.user_id = uid;
    v_was_reset := true;
  END IF;
  RETURN QUERY SELECT uc.credits_remaining, uc.credits_used, v_was_reset FROM public.user_credits uc WHERE uc.user_id = uid;
END; $function$;
