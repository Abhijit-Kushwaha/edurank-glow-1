...
-- Fix: set existing pending orgs to active
UPDATE public.organisations SET status = 'active' WHERE status = 'pending';

-- Fix: change default status from 'pending' to 'active'
ALTER TABLE public.organisations ALTER COLUMN status SET DEFAULT 'active';

-- Recreate create_organisation to set status = 'active' explicitly
