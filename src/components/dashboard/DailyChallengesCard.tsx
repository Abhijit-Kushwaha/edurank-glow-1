      ...
      <Card className="bg-gradient-to-br from-orange-500/10 to-yellow-500/10 border-orange-500/20">
        <CardContent className="py-6 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Complete quizzes to start your challenges!
          </p>
        </CardContent>
      </Card>
    );
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base flex items-center gap-2">
            <Flame
              className={`h-5 w-5 ${allCompleted ? "text-green-500" : "text-orange-500"}`}
            />
            Daily Challenges
          </CardTitle>
          <div className="flex items-center gap-2">
            {totalXpEarned > 0 && (
              ...
              >
                <Gift className="h-3 w-3 mr-1" />+{totalXpEarned} XP
              </Badge>
            )}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <p
              className={`font-medium text-sm ${challenge.is_completed ? "line-through text-muted-foreground" : "text-foreground"}`}
            >
              {info.title}
            </p>
            <Badge
              variant="outline"
              className={`text-[10px] px-1.5 py-0 ${difficultyClasses}`}
            >
              {info.difficulty}
            </Badge>
          </div>
