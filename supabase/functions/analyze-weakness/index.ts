...
async function searchYouTubeForTopic(topic: string, apiKey: string): Promise<YouTubeVideo | null> {
  try {
    ...
    if (!response.ok) { console.error("YouTube search error:", response.status); return null; }
    const data = await response.json();
    const item = data.items?.[0];
    if (item) return { videoId: item.id.videoId, title: item.snippet.title, channel: item.snippet.channelTitle };
    return null;
  } catch (error) { console.error("Error searching YouTube:", error); return null; }
}

function computeWeaknessScore(accuracy: number, avgTimeUser: number, avgTimeGlobal: number, repeatedMistakes: number, wrongOnEasy: number, wrongOnMedium: number, wrongOnHard: number): number {
  const accuracyPenalty = (1 - accuracy) * 50;
  let timePenalty = 0;
  if (avgTimeGlobal > 0 && avgTimeUser > avgTimeGlobal) timePenalty = Math.min(20, ((avgTimeUser - avgTimeGlobal) / avgTimeGlobal) * 20);
  const consistencyPenalty = repeatedMistakes >= 2 ? 15 : 0;
  let difficultyPenalty = 0;
  if (wrongOnHard > 0) difficultyPenalty = 15;
  else if (wrongOnMedium > 0) difficultyPenalty = 8;
  else if (wrongOnEasy > 0) difficultyPenalty = 5;
...
serve(async (req) => {
  ...
  try {
    ...
    const rateLimitResult = await checkRateLimit(supabaseClient, { operation: "analyze-weakness", userId, limitsPerHour: 3, limitsPerDay: 15 });
    if (!rateLimitResult.allowed) return withCorsError(req, 429, rateLimitResult.message || "Rate limit exceeded");

    const creditResult = await consumeCredits(userId, CREDIT_COSTS["analyze-weakness"]);
    ...
    if (!todoId || !videoId || !questions || !Array.isArray(questions)) return withCorsError(req, 400, "Missing required fields: todoId, videoId, questions");
    ...
    try {
      ...
      const topicsText = await callLovableAI([
        { role: "system", content: `You are an educational topic classifier. For each question, identify the main concept/topic being tested.\nReturn a JSON array of topic names, one for each question.\nTopics should be concise (2-4 words), educational, and specific.\nIMPORTANT: Return ONLY a valid JSON array of strings, nothing else.` },
        { role: "user", content: `Extract the main topic for each of these ${questions.length} questions (separated by ---):\n\n${questionTexts}` },
      ]);
      const jsonMatch = topicsText.match(/\[[\s\S]*\]/);
      if (jsonMatch) {
        const topics = JSON.parse(jsonMatch[0]);
        questions.forEach((q: any, i: number) => { questionsWithTopics.push({ ...q, topicName: topics[i] || "General Knowledge" }); });
      }
    } catch (aiError) { console.error("AI topic extraction failed:", aiError); }

    if (questionsWithTopics.length === 0) {
      questions.forEach((q: any) => { questionsWithTopics.push({ ...q, topicName: "General Knowledge" }); });
    }
    ...
    for (const topicName of uniqueTopics) {
      let { data: topic } = await serviceClient.from("topics").select("id").eq("name", topicName).maybeSingle();
      if (!topic) {
        const { data: newTopic, error } = await serviceClient.from("topics").insert({ name: topicName }).select("id").single();
        if (error) { console.error("Error creating topic:", error); continue; }
        topic = newTopic;
      }
      if (topic) topicIdMap[topicName] = topic.id;
    }

    // Step 3: Get attempt number
    const { count: existingAttempts } = await supabaseClient.from("question_attempts").select("*", { count: "exact", head: true }).eq("todo_id", todoId);
    ...
    const attemptsToInsert = questionsWithTopics.map((q) => ({
      user_id: userId, quiz_id: quizId, todo_id: todoId, video_id: videoId,
      topic_id: topicIdMap[q.topicName || "General Knowledge"] || null,
      question_text: q.questionText, is_correct: q.isCorrect,
      time_taken_seconds: q.timeTakenSeconds || 0,
      difficulty_level: q.difficulty || "medium", attempt_number: attemptNumber,
    }));
    const { error: insertError } = await supabaseClient.from("question_attempts").insert(attemptsToInsert);
    if (insertError) console.error("Error inserting attempts:", insertError);
    ...
    questionsWithTopics.forEach((q) => {
      ...
      if (!topicPerformance[topicId]) {
        topicPerformance[topicId] = { topicId, topicName, totalQuestions: 0, correctAnswers: 0, totalTimeSeconds: 0, wrongOnEasy: 0, wrongOnMedium: 0, wrongOnHard: 0, repeatedMistakes: 0 };
      }
      const perf = topicPerformance[topicId];
      perf.totalQuestions++;
      if (q.isCorrect) perf.correctAnswers++;
      else {
    ...
    for (const [topicId, perf] of Object.entries(topicPerformance)) {
      const { data: existingPerf } = await supabaseClient.from("user_topic_performance").select("*").eq("topic_id", topicId).maybeSingle();
      const { data: globalStats } = await serviceClient.from("global_topic_stats").select("*").eq("topic_id", topicId).maybeSingle();
      ...
      if (!perf.correctAnswers && existingPerf && !existingPerf.correct_answers) repeatedMistakes++;
      ...
      const weaknessScore = computeWeaknessScore(accuracy, newAvgTime, globalAvgTime, repeatedMistakes, newWrongEasy, newWrongMedium, newWrongHard);
      ...
      await supabaseClient.from("user_topic_performance").upsert({
        user_id: userId, topic_id: topicId, total_questions: newTotalQuestions,
        correct_answers: newCorrectAnswers, total_time_seconds: newTotalTime,
        avg_time_seconds: newAvgTime, wrong_on_easy: newWrongEasy,
        wrong_on_medium: newWrongMedium, wrong_on_hard: newWrongHard,
        repeated_mistakes: repeatedMistakes, weakness_score: weaknessScore,
        strength_status: strengthStatus, last_updated: new Date().toISOString(),
      ...
      await supabaseClient.from("video_topic_analysis").upsert({
        user_id: userId, video_id: videoId, todo_id: todoId, topic_id: topicId,
        questions_count: perf.totalQuestions, correct_count: perf.correctAnswers,
        mastery_score: videoAccuracy * 100, is_weak_topic: isWeakInVideo,
      ...
      await serviceClient.from("global_topic_stats").upsert({
        ...
        avg_accuracy: (((globalStats?.total_correct || 0) + perf.correctAnswers) / ((globalStats?.total_attempts || 0) + perf.totalQuestions)) * 100,
        avg_time_seconds: ((globalStats?.avg_time_seconds || 30) * (globalStats?.total_attempts || 1) + perf.totalTimeSeconds) / ((globalStats?.total_attempts || 0) + perf.totalQuestions),
        updated_at: new Date().toISOString(),
      }, { onConflict: "topic_id" });

      if (strengthStatus === "weak") {
        weakTopics.push({ topicId, topicName: perf.topicName, weaknessScore, accuracy: accuracy * 100 });
        const youtubeApiKey = Deno.env.get("youtube_api_key");
        let videoData: YouTubeVideo | null = null;
        if (youtubeApiKey) {
          videoData = await searchYouTubeForTopic(perf.topicName, youtubeApiKey);
          if (videoData) console.log(`Found video for weak topic "${perf.topicName}": ${videoData.title}`);
        }
        const minutes = Math.max(3, Math.ceil(weaknessScore / 20));
        recommendations.push({
          user_id: userId, topic_id: topicId, todo_id: todoId,
          recommendation_type: "weak_topic_quiz",
          title: `Fix: ${perf.topicName}`,
          ...
          priority: Math.round(weaknessScore), weakness_score: weaknessScore,
          video_id: videoData?.videoId || null, video_title: videoData?.title || null,
      ...
      if (previousScore > 0 && weaknessScore > 0) {
        ...
        if (improvement >= 30) await serviceClient.rpc("check_achievements", { uid: userId });
      }
    }

    if (recommendations.length > 0) {
      const topicIds = recommendations.map((r) => r.topic_id);
      await supabaseClient.from("recommendation_queue").delete().eq("user_id", userId).in("topic_id", topicIds);
      await supabaseClient.from("recommendation_queue").insert(recommendations);
    }

    const { data: allResults } = await supabaseClient.from("quiz_results").select("score, correct_answers, total_questions").order("created_at", { ascending: false });
    if (allResults && allResults.length > 0) {
      ...
      await supabaseClient.from("leaderboard_stats").upsert({
        user_id: userId, total_quizzes: totalQuizzes, total_correct: totalCorrect,
        total_questions: totalQuestions, average_score: Math.round(averageScore),
        best_score: bestScore, last_activity_date: new Date().toISOString().split("T")[0],
        updated_at: new Date().toISOString(),
      }, { onConflict: "user_id" });
    }
