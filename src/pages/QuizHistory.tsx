...
const QuizHistory = () => {
  ...
  const calculateStats = () => {
    if (results.length === 0)
      return { average: 0, best: 0, total: 0, trend: 0 };
    ...
    let trend = 0;
    if (results.length >= 4) {
      const recent = scores.slice(
        0,
        Math.min(5, Math.floor(scores.length / 2)),
      );
      ...
      <main className="container mx-auto px-4 py-6 space-y-6">
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-fade-in">
          ...
          <div className="glass-card rounded-xl p-4">
            ...
            <p
              className={`text-2xl font-bold ${stats.trend > 0 ? "text-success" : stats.trend < 0 ? "text-destructive" : ""}`}
            >
              {stats.trend > 0 ? "+" : ""}
          ...
          {results.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Complete a study task and take a quiz to see your history
              </p>
